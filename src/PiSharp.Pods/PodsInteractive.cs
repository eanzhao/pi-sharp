...
internal sealed class PodsInteractiveView : Component, IInputComponent, IFocusableComponent
{
    private readonly Input _input;
    private string _status = "PiSharp Pods";
    private string _transcript = string.Empty;

    public PodsInteractiveView(string prompt = "> ", string? placeholder = null)
    {
...
internal sealed class PodsInteractiveController
{
    private readonly PiSharp.Agent.Agent _agent;
    private readonly PodsInteractiveView _view;
    private readonly PodEndpoint _endpoint;
    ...
    public PodsInteractiveController(PiSharp.Agent.Agent agent, PodsInteractiveView view, PodEndpoint endpoint)
    {
    ...
    public async Task SubmitAsync(string prompt, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await _agent.PromptAsync(prompt, cancellationToken: cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            _isBusy = false;
    ...
    {
        var activity = _isBusy ? "running" : "idle";
        _view.Status = $"PiSharp Pods | {_endpoint.PodName}/{_endpoint.ModelId} | {activity}";
    }
...
internal static class PodsConsoleKeyMapper
{
