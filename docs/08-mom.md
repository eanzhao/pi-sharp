...
- `<workspace>/slack-metadata.json`
- `<workspace>/<channel>/log.jsonl`
- `<workspace>/<channel>/MEMORY.md`
- `<workspace>/<channel>/scratch/`
- `<workspace>/<channel>/.pi-sharp/sessions/*.jsonl`

phase 08 现在也提供了一个只读入口：`pisharp mom stats <workspace>`（或 `PiSharp.Mom` 直接 `stats <workspace>`），把 `runtime-stats.json` 展开成人类可读的摘要，方便现场查看最近一次 backfill / reconnect 成功或失败，而不用手翻 JSON；如果要机器读，也支持 `pisharp mom stats --json <workspace>`，当前顶层会带一个稳定的 `schemaVersion`、`requestedChannel` / `resolvedChannelId`，以及 workspace / channel 级 `paths`，方便外部脚本直接定位 `runtime-stats.json`、`slack-metadata.json`、`log.jsonl` 和 session / attachment 目录。另一方面，bot 运行时会把 users/channels metadata 也持久化到 `slack-metadata.json`，所以 `mom stats --channel C123 <workspace>` 或 `mom stats --channel general <workspace>` 离线都能显示 channel label、用户名，以及 metadata 的 `refreshedAt`、age 和 `fresh/stale` 状态，便于判断这些名字是不是过期了。
